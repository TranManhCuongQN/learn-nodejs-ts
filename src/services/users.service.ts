...
class UsersService {
  ...
  private signRefreshToken({ user_id, verify, exp }: { user_id: string; verify: UserVerifyStatus; exp?: number }) {
    // nếu có exp mình sẽ tạo ra 1 refresh token có thời gian sống là exp
    if (exp) {
      return signToken({
        payload: {
          user_id,
    ...
    // nếu không có exp mình sẽ tạo ra 1 refresh token có thời gian sống là thời gian mặc định
    return signToken({
      payload: {
        user_id,
        token_type: TokenType.RefreshToken,
  ...
  async register(payload: RegisterReqBody) {
    ...
    // lấy ra iat, exp từ refresh token cũ để lưu vào database. Để sau này tạo ra refresh token mới có thời gian sống như cũ
    const { iat, exp } = await this.decodeRefreshToken(refresh_token)
